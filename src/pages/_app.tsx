...
import { graphql, useRelayEnvironment, RelayEnvironmentProvider } from "relay-hooks";
            ...
            <RelayEnvironmentProvider environment={getEnvironment()}>
                {!useIsUserLoggedIn(router.route.split("/")[1]) ? <Component {...pageProps} /> :
                    <QueryRenderer<AppWrapperQuery>
                        ...
                        variables={{}}
                        render={
                            ({ error, props, retry }: { error: Error, props: AppWrapperQueryResponse, retry: () => void }) => {
                                if (error) {
                                    showNotification("Please login to Continue", "error")
                                    return null
                                } else if (props) {
                                    return <Component {...pageProps} viewer={props.getPlayer} refetch={retry} />
