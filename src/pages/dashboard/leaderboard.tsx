...
import { Box, Grid, TextField, Tooltip, Paper, Card, CardActionArea, CardMedia, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from '@material-ui/core';
...
const LeaderBoard: NextPage<ProtectedPageProps> = ({
    viewer
}) => {

    const classes = useStyles()

    const { data, error, retry, isLoading } = useQuery<GetLeaderBoardQuery>(query)
    ...
    return (
        <div>
            ...
            {    Boolean(error) ?
                ...
                isLoading ? <Box className={classes.loading}><LoadingScreen loading={isLoading} /> </Box> :
                    <>
                        <div className={classes.root}>
                            <TableContainer component={Paper}>
                                <Table aria-label="caption table">
                                    <caption>Need to solve at-least 1 question to come im leader board</caption>
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <TableCell align="center"><b>Questions Solved</b></TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {data.getLeaderBoard.map((row, index) => (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell align="center">{row.questionAttempted}</TableCell>
